...
*base image* - базовый образ, не имеет родительского образа. Обычно это образы с операционной
системой

*child image* - дочерний образ, построен на базовом образе и обладает дополнительной
функциональностью

Так же есть официальные образы (поддерживаются командой docker, обычно в названии имеют только одно
слово) и пользовательские образы.
...
*daemon* - демон докера, фоновый процесс, который управляет Docker-объектами: сетями, хранилищами,
